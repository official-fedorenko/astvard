...
namespace AstvardServerMod
{
    public partial class Plugin
    {
        ...
        private static void TickGarden(Player player, Sorting.Zone zone, List<Container> supply)
        {
            ...
            // Огород, выключенный целиком, не стоит двух обходов сцены. Оба сеятеля
            // выходят первой строкой и так, а `ReapBeds` доходила до `FindObjectsByType`
            // ради счётчиков, которые в этом случае читает только предел - а предел
            // спрашивают те же сеятели. Проверять надо все три: при выключенном сборе, но
            // включённом засеве счёт грядок по-прежнему нужен.
            if (!ReapOn && !SowOn && !SowEmptyOn)
            {
                BedsInZone = 0;
                BedsFull = false;
                return;
