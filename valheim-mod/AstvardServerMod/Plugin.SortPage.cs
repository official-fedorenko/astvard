...
namespace AstvardServerMod
{
    public partial class Plugin
    {
        ...
        internal static GameObject SortTreesButton;
        ...
        private static void StartSortZonePreview()
        {
            ...
            CancelOtherPreviews(Projection.SortZone);
        ...
        private void CreateSortSettingWidgets(Jotunn.Managers.GUIManager gui)
        {
            ...
            SortTreesButton = MakeButton(gui, "", () =>
            {
                SetSowTrees(!SowTreesOn);
                Player.m_localPlayer?.Message(MessageHud.MessageType.Center,
                    SowTreesOn
                        ? "Деревья тоже сажаем — им вскопанная земля не нужна"
                        : "Деревья не сажаем");
                RefreshMenu();
            });
        ...
        private static void RefreshSortVisibility()
        {
            ...
            // Полки уехали в своё окно по центру экрана, а три флажка остались в панели,
            // под «Назад»: ими помечают чаще всего, и держать их в двух местах разом -
            // это две стены одинаковых кнопок на одном экране.
