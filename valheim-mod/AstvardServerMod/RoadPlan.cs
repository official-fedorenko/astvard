...
namespace AstvardServerMod
{
    ...
    internal static class RoadPlan
    {
        ...
        /// <summary>
        /// Скругляет углы, не двигая концов и не выпуская дорогу в непроходимое.
        ///
        /// Ступеньки сетки убирает <see cref="Simplify"/>, но после него дорога - ломаная
        /// с углами по 45°, и в игре такой угол виден изломом. Здесь каждая точка съезжает
        /// к середине между соседями, и так много раз подряд.
        ///
        /// **Считать это надо на густом пути, а не на ломаной.** Один и тот же шаг на
        /// точках через метр даёт дугу радиусом около `sqrt(passes * pull)` метров, а на
        /// точках через полсотни - швыряет вершину на двадцать метров вбок. Прежняя
        /// `Round` делала второе и оттого не столько скругляла, сколько сдвигала дорогу.
        ///
        /// **Точка, попавшая бы в непроходимое, остаётся на месте.** Сглаживание тянет
        /// путь к хорде, то есть срезает углы, а срезанный угол у валуна - это дорога
        /// сквозь валун. Проверка стоит внутри прохода, а не после него: откатить одну
        /// точку дешевле, чем отказаться от сглаживания целиком.
        /// </summary>
        internal static List<Vec2> Ease(Field field, IList<Vec2> path, int passes, float pull)
        {
            var now = new List<Vec2>(path);
            if (field == null || now.Count < 3) return now;

            // Концы в обоих списках стоят с самого начала и не пишутся никогда, так что
            // после обмена местами они на месте в любом из них.
            var next = new List<Vec2>(now);
            ...
            for (var pass = 0; pass < passes; pass++)
            {
                for (var i = 1; i < now.Count - 1; i++)
                {
                    ...
                    var moved = new Vec2(now[i].X + (middleX - now[i].X) * pull,
                                         now[i].Z + (middleZ - now[i].Z) * pull);

                    int x, z;
                    next[i] = field.Cell(moved, out x, out z) && field[x, z] < Blocked
                        ? moved
                        : now[i];
                }
