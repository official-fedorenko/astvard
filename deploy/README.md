...
меняют, а конфиг nginx перед заменой сохраняют и при ошибке возвращают. Общий Postgres
сайту сейчас не нужен: панель живёт на SQLite. Перевод на Postgres — отдельная работа,
и до неё чужой контейнер не трогаем.
...
недостающие ключи, ставит зависимости, кладёт systemd-юнит и меняет nginx. В конце —
живой запрос к `/api/health`; не ответил, скрипт падает и печатает журнал.

**База лежит вне папки с кодом** — `/srv/astvard-data/db.sqlite`, путь в `DB_PATH`.
Обновление делает `git reset --hard`, и однажды кто-нибудь потянется за `git clean -xdf`;
за пределами репозитория базу это не достанет. Схему панель ведёт сама, миграциями при
старте, так что отдельного шага для неё в деплое нет.

**Ключи `.env`**, которые дописывает установка: `PORT=3001`, `APP_URL`, `DB_PATH`,
`TRUST_PROXY=true` (nginx терминирует HTTPS, без этого кука сессии не получит флаг
`Secure`), `VALHEIM_LOG_FILE` и `SUPERADMIN_STEAM_ID`. Уже существующие значения скрипт
не трогает: секрет, который подписывает сессии, менять на ходу нельзя.
...
живут только `.env`, `node_modules`, `uploads/` и база в `/srv/astvard-data`.
