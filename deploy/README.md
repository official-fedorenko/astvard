...
**Первый суперадмин заводится сам.** В `.env` есть `SUPERADMIN_STEAM_ID`: при каждом
старте бэкенд смотрит, есть ли на сайте хоть один суперадмин, и если нет — заводит или
повышает аккаунт с этим SteamID, сразу с доступом на сервер и админкой в игре. Хозяину
остаётся нажать «Войти через Steam».

```bash
SUPERADMIN_STEAM_ID=76561198XXXXXXXXX POSTGRES_CONTAINER=postgres-shared ./deploy/install-site.sh
# уже установленному: вписать строку в /srv/astvard/.env, затем
systemctl restart astvard-backend
journalctl -u astvard-backend -n 20 --no-pager   # ждём «Суперадмин заведён: …»
```

Пароля для этого нет намеренно: вход на сайт идёт через Steam, а пароль в `.env` — это
пароль, лежащий на диске. Повторный старт ничего не трогает, пока на сайте есть хоть
один суперадмин: осознанное разжалование перезапуск не отменяет.
...
**Клон на VPS — только читатель.** Обновление делает `git reset --hard
origin/valheim-mod`, поэтому правка, сделанная прямо на сервере, исчезнет при
следующем обновлении, и никакого следа от неё не останется. Всё идёт одним путём:
правка на машине разработчика → `git push` → `update-site.sh` здесь. Коммитить на VPS
не надо никогда; вне git там только `.env`, `deploy/valheim/.env` и `node_modules`.
