...
оттуда берутся руны и общие постройки для раздела «Наш сервер»), `GAME_LISTS_TOKEN`
(случайный токен, по которому мод игрового сервера забирает списки доступа с
`/api/game/lists`; тот же вписывается моду в `[Сайт] ListsToken`, а `ListsUrl` —
адрес этого маршрута) и `SUPERADMIN_STEAM_ID`. Уже существующие
...
бы и чужие сайты на этой машине. Там же `www.astvard.online` и весь http уводятся одним
301 на `https://astvard.online`: для поисковика два имени — две копии каждой страницы.
Сертификат `www` уже покрывает, а certbot продлевает его через nginx-плагин, так что
отдельный блок для `www` продлению не мешает.
...
Файлы для него собирает сайт: «Админка → Серверы → Создать новый сервер». Оттуда
выходят `server.env`, `compose.yml` и команды по шагам — сайт их только пишет,
выполняет хозяин. Живёт такой сервер в `/srv/valheim-<ключ>`, своей установкой игры
и своей парой портов (2458–2459 для первого из них); образ берётся готовый,
`astvard-valheim:local`. Какие флаги вообще бывают и почему в форме нет сида — в
корневом `CLAUDE.md`, раздел «Создание второго сервера с сайта».

Чтобы сайт видел его статус, сервер добавляется в тот же список: адрес, порт,
«Статус: лог Valheim» и **ключ** — по ключу сайт и находит `/srv/valheim-<ключ>/logs/server.log`.
