...
function renderWhitelist(user) {
  ...
  if (!user.steam_id) {
    ...
    lines.push('<a class="btn btn-steam" href="/api/auth/steam">Привязать Steam</a>');
  ...
  if (status === 'none' || status === 'rejected') {
    ...
    lines.push(`
      ...
      <button id="wl-request" class="btn btn-primary">${status === 'rejected' ? 'Подать заново' : 'Запросить доступ'}</button>
    `);
  }
  if (status === 'pending') {
  ...
  lines.push(`<div class="card-footer">
    <span class="muted">Сменил аккаунт Steam? Доступ придётся запросить заново.</span>
    <a class="btn btn-sm btn-ghost" href="/api/auth/steam">Привязать другой</a>
  </div>`);
