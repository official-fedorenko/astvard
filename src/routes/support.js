...
// Имя, которым подписаны ответы администрации. Не кэшируем — чат отвечает редко
// относительно чтения.
function getChatDisplayName(cb) {
  db.get(
    "SELECT value FROM settings WHERE key = 'support_admin_display_name'",
    [],
    (err, row) => cb((row && row.value) || 'Администрация')
  );
}

// Подменяет name у сообщений от админов на общее имя администрации, не трогая
// хранимые данные. Сообщения игроков не меняются.
function applyDisplayNames(messages, cb) {
  getChatDisplayName((adminDisplayName) => {
    messages.forEach((m) => {
  ...
  if (currentPath === '/api/support/tickets' && method === 'GET') {
    ...
    db.all(query, [], (err, rows) => {
      ...
      db.all("SELECT ticket_id, status FROM support_tickets", [], (e1, statusRows) => {
        ...
        rows.forEach((r) => { delete r.owner_user_id; });
