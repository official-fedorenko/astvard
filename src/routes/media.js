...
// Для набора URL'ов определяет, к какому пользователю привязан каждый файл
// (аватар). Возвращает объект { [file_url]: attachment } через колбэк.
function resolveAttachments(urls, cb) {
  ...
  db.all(`SELECT avatar_url AS url, username FROM users WHERE avatar_url IN (${ph})`, unique, (err, userRows) => {
    (userRows || []).forEach(r => {
